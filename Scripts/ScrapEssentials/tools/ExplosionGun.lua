...
function ExplosionGun.client_onCreate( self )
...
function ExplosionGun.client_onRefresh( self )
	self:loadAnimations()
end

function ExplosionGun.loadAnimations( self )
...
function ExplosionGun.client_onUpdate( self, dt )
...
function ExplosionGun.client_onEquip( self )
...
function ExplosionGun.client_onUnequip( self )
	self.wantEquipped = false
	self.equipped = false
...
function ExplosionGun.server_network_onAim( self, aiming )
	self.network:sendToClients( "client_network_onAim", aiming )
end

function ExplosionGun.client_network_onAim( self, aiming )
	if not self.tool:isLocal() then
		self:onAim( aiming )
	end
end

function ExplosionGun.onAim( self, aiming )
	self.aiming = aiming
...
function ExplosionGun.server_network_onShoot( self, dir ) 
	--print(self.tool:getOwner(), dir)
	local success, result = sm.physics.raycast( self.tool:getOwner().character.worldPosition, self.tool:getOwner().character.worldPosition + dir*100)
	if success then
		sm.physics.explode( result.pointWorld, 5, 5, 5, 5, "PropaneTank - ExplosionSmall")
	end
	self.network:sendToClients( "client_network_onShoot", dir )
end

function ExplosionGun.client_network_onShoot( self, dir ) 
	if not self.tool:isLocal() then
		self:onShoot( dir )
	end
end

function ExplosionGun.onShoot( self, dir ) 
...
function ExplosionGun.calculateFirePosition( self )
...
function ExplosionGun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )		
	local right = sm.localPlayer.getRight()
...
function ExplosionGun.calculateFpMuzzlePos( self )
...
function ExplosionGun.client_onPrimaryUse( self, state )
...
function ExplosionGun.client_onSecondaryUse( self, state )
