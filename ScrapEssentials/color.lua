...
function sm.color.toRGB( hsv )
	...
	local M = hsv.v - C
	local H = (math.round( hsv.h / 60 ) + 1) % 6
	local out_rgb
	local rgb = {}
	rgb[1] = function( C, X ) return { r = C, g = X, b = 0 } end
	rgb[2] = function( C, X ) return { r = X, g = C, b = 0 } end
	rgb[3] = function( C, X ) return { r = 0, g = C, b = X } end
	rgb[4] = function( C, X ) return { r = 0, g = X, b = C } end
	rgb[5] = function( C, X ) return { r = X, g = 0, b = C } end
	rgb[6] = function( C, X ) return { r = C, g = 0, b = X } end
	out_rgb = rgb[H]( C, X )
